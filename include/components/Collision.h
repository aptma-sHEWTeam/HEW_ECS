    ...
    void OnUpdate(World &w, Entity self, float dt) override {
        ...
        for (const auto &collidable_a : collidables) {
            ...
            for (Entity e_b : potentialColliders) {
                ...
                if (!w.IsAlive(e_b) || e_a.id >= e_b.id)
                    continue;
 ...
 * @brief 特定のハンドラー型を試行（テンプレート宣言のみ）
    ...
    /**
     * @brief OBB(回転考慮) vs Sphere 衝突判定
     * @details Transformの回転（Yaw）を考慮した衝突判定。斜め壁コライダーに対応。
     */
    static std::optional<CollisionInfo> CheckAABB_Sphere(
        ...
        // ボックスの回転を取得（Yaw回転のみ考慮）
        float yawRad = XMConvertToRadians(tBox.rotation.y);
        XMVECTOR rotQuat = XMQuaternionRotationRollPitchYaw(0.0f, yawRad, 0.0f);
        XMVECTOR invRotQuat = XMQuaternionConjugate(rotQuat);

        // 球の中心をボックスのローカル座標系に変換
        XMVECTOR vBoxCenter = XMLoadFloat3(&boxCenter);
        XMVECTOR vSphereCenter = XMLoadFloat3(&sphereCenter);
        XMVECTOR relativePos = XMVectorSubtract(vSphereCenter, vBoxCenter);
        XMVECTOR localSpherePos = XMVector3Rotate(relativePos, invRotQuat);

        XMFLOAT3 localSphere;
        XMStoreFloat3(&localSphere, localSpherePos);

        // ローカル座標系でのAABB判定
        XMFLOAT3 halfSize = {boxSize.x * 0.5f, boxSize.y * 0.5f, boxSize.z * 0.5f};
        XMFLOAT3 localClosest = {
            std::clamp(localSphere.x, -halfSize.x, halfSize.x),
            std::clamp(localSphere.y, -halfSize.y, halfSize.y),
            std::clamp(localSphere.z, -halfSize.z, halfSize.z)};

        // ローカル座標系での距離計算
        float dx = localSphere.x - localClosest.x;
        float dy = localSphere.y - localClosest.y;
        float dz = localSphere.z - localClosest.z;
        ...
        if (logCounter++ < 500) {
            ...
            if (distSq < rSq + 1.0f) { // ニアミスも含めてログ出力
                sprintf_s(buf, "CheckOBB: distSq=%.4f, rSq=%.4f, boxY=%.2f", distSq, rSq, boxSize.y);
        ...
        if (distSq < radius * radius) {
            ...
            // 最近傍点をワールド座標に変換
            XMVECTOR vLocalClosest = XMLoadFloat3(&localClosest);
            XMVECTOR vWorldClosest = XMVectorAdd(XMVector3Rotate(vLocalClosest, rotQuat), vBoxCenter);
            XMStoreFloat3(&info.contactPoint, vWorldClosest);

            // 法線をワールド座標に変換
            if (dist > 1e-6f) {
                XMFLOAT3 localNormal = {dx / dist, dy / dist, dz / dist};
                XMVECTOR vLocalNormal = XMLoadFloat3(&localNormal);
                XMVECTOR vWorldNormal = XMVector3Rotate(vLocalNormal, rotQuat);
                XMStoreFloat3(&info.normal, vWorldNormal);
            } else {
                // 球の中心がBox内にある場合、最も近い面への法線を採用
                float distX = halfSize.x - std::abs(localSphere.x);
                float distY = halfSize.y - std::abs(localSphere.y);
                float distZ = halfSize.z - std::abs(localSphere.z);

                if (distX < distY && distX < distZ) {
                    info.normal = {localSphere.x > 0 ? 1.0f : -1.0f, 0, 0};
                } else if (distY < distZ) {
                    info.normal = {0, localSphere.y > 0 ? 1.0f : -1.0f, 0};
                } else {
                    info.normal = {0, 0, localSphere.z > 0 ? 1.0f : -1.0f};
                }

                XMVECTOR vLocalNormal = XMLoadFloat3(&info.normal);
                XMVECTOR vWorldNormal = XMVector3Rotate(vLocalNormal, rotQuat);
                XMStoreFloat3(&info.normal, vWorldNormal);
            }
...
struct CollisionHandlerAutoRegister {
    CollisionHandlerAutoRegister() {
        CollisionHandlerRegistry::RegisterType(std::type_index(typeid(T)), +[](World &w, Entity e, const std::function<void(ICollisionHandler *)> &func) {
