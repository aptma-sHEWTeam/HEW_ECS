...
inline static ConfigVar<float> cfg_MinSpeed{"Player", "MinSpeedFactor", 1.0f};
inline static ConfigVar<float> cfg_AccelerateMagnification{"Player", "AccelerateMagnification", 1000.0f};
...
struct PlayerVelocity : Behaviour {
    ...
    float MinSpeed = cfg_MinSpeed;                    ///< 減速時の最小速度
    ...
    void UpdatePosition(World &w, Entity self, float dt) {
        ...
        if (isBoosting) {
            ...
            DEBUGLOG("加速速度 X: " + std::to_string(velocity.x));
            DEBUGLOG("加速速度 Y: " + std::to_string(velocity.y));
            isBoosting = false;
        }
