    ...
    if (isFading) {
        ...
        world.ForEach<GameStatus>([&](Entity, GameStatus &stats) {
            ...
            if (togglePause) {
                ...
                TriggerGamepadRumble(std::clamp(cfg_ControllerRumbleSubmitStrength.Get(), 0.0f, 1.0f),
                                     std::clamp(cfg_ControllerRumbleSubmitStrength.Get(), 0.0f, 1.0f),
